...
from PyQt5.QtWidgets import (QApplication, QCheckBox, QComboBox,
                             QDoubleSpinBox, QFileDialog, QLabel, QMenu,
                             QMenuBar, QMessageBox, QPushButton, QSlider,
                             QSpinBox, QStatusBar, QStyleFactory, QWidget)
...
class MainWindowUI:
    ...
    def setupUi(self, MainWindow):
        ...
        self.checkBox_prenormalized.setFont(self.getFont(pointsize=12, bold =False, weight=50))
        ...
        self.label_geometries = [('Width:', (190, 10, 43, 25)),
                            ('Height:', (190, 45, 51, 25)),
        ...
        for i, (_, geo) in enumerate(self.label_geometries):
            name = f"label_{i}"
            setattr(self, name, self.getLabel(self.centralwidget, geo, label_font, name))
        ...
        #comboBox_matrix
        self.comboBox_matrix.currentIndexChanged['QString'].connect(self.get_row_and_col)
        #change style
    ...
    def savedata(self):
        ...
        if self.outputdata is not None:
            ...
            df = pd.DataFrame(self.outputdata, 
        ...
        if self.roicol is not None:
            if len(self.roicol.roidict) != 0:
                try:
                    ...
                    self.showMessage("[SYSTEM] Roi.json was saved at %s" % self.imagedir)
    ...
    def overlaythreshold(self, img, val):
        ...
        retval, binaryimg = cv2.threshold(img[:,:,2],threshold,1,
                                           cv2.THRESH_BINARY_INV).astype(bool)
